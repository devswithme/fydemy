...
export default function ApiReference() {
  ...
  const onSubmit = () => {
    console.log(form);
    if (form.one == 'b' && form.two == 'b' && form.three == 'a' && form.four == 'c' && form.five == 'b') {
      setForm({
        one: '',
        two: '',
        three: '',
    ...
    <main className="max-w-3xl mx-auto prose">
      ...
      <RadioGroup className="-space-y-10" value={form.one} onValueChange={(value) => handleChange('one', value)}>
        ...
        <label className="flex items-center space-x-3">
          ...
          <p>Paket data yang dikirim dari komponen induk ke komponen anak</p>
        </label>
        <label className="flex items-center space-x-3">
      ...
      <p className="mb-0">2. Bagaimana cara mengirim props ke komponen anak?</p>
      <RadioGroup className="-space-y-10" value={form.two} onValueChange={(value) => handleChange('two', value)}>
        ...
        <label className="flex items-center space-x-3">
          ...
          <p>Dengan memberikan atribut saat memanggil komponen anak</p>
        </label>
        <label className="flex items-center space-x-3">
          <RadioGroupItem value="c" id="two-c" />
          <p>
            Dengan menulis <code>{`props.nama = "Budi"`}</code> langsung di dalam komponen anak
          </p>
        </label>
      </RadioGroup>

      <p className="mb-0">3. Apa yang akan ditampilkan oleh kode ini?</p>
      <pre>
...
console.log(add(3, 4, 5))`}</code>
      ...
      <p className="mb-0">5. Kenapa props tidak boleh diubah oleh komponen anak?</p>
      <RadioGroup className="-space-y-10" value={form.five} onValueChange={(value) => handleChange('five', value)}>
        ...
        <label className="flex items-center space-x-3">
          ...
          <p>Karena props adalah satu arah (one-way binding) dari induk ke anak</p>
        </label>
        <label className="flex items-center space-x-3">
          <RadioGroupItem value="c" id="five-c" />
          <p>Karena komponen anak tidak boleh punya variabel sendiri</p>
        </label>
      </RadioGroup>
